...
export interface BacklogRow {
  ...
  converted_demo_ref:   string | null
...
export class BacklogRepository {
  ...
  static async markConverted(id: number, demoPk: number, demoRef?: string | null): Promise<BacklogRow | null> {
    const result = await pool.query<BacklogRow>(
      `UPDATE public.demo_backlog
       SET status = 'Converted', converted_demo_id = $1, converted_demo_ref = $2,
           converted_at = NOW(), updated_at = NOW()
       WHERE id = $3
       RETURNING *`,
      [demoPk, demoRef ?? null, id],
    )
    return result.rows[0] ?? null
  }
