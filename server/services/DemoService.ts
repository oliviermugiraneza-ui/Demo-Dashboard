...
export class DemoService {
  ...
  static normaliseStatus(raw: string | null): string {
    ...
    if (s.includes('TO DELETE') || s.includes('DELETE')) return 'DELETED'
    if (s === 'REVIEWED')                                 return 'Reviewed'
    if (s.includes('NEEDS'))                              return 'Needs Review'
  ...
  static async createDemo(input: CreateDemoInput): Promise<void> {
    await DemoRepository.create(input)
  }
