...
export default async function createDemoRequest(
  ...
  const result = await retoolDb.query<{ id: string }>(
    `INSERT INTO demo_raw_data (
       ...
       requester, approver, created_at, updated_at
     ) VALUES (
    ...
    [
      normalize(p.action) ?? 'NEEDS REVIEW',
      normalize(p.status) ?? 'pending',
      p.geo, p.demo_type,
      ...
      normalize(p.length),
      normalize(p.requester) ?? user.fullName,
      normalize(p.approver),
    ],
  )
