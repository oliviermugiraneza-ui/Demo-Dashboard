...
interface BacklogCompletedTableProps {
  items:        BacklogItem[]
...
export default function BacklogCompletedTable({ items, onSelectItem }: BacklogCompletedTableProps) {
  ...
  return (
    <div className="rounded-xl border border-gray-100 shadow-sm bg-white overflow-hidden">
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100" style={{ background: '#ecf6ee' }}>
        <div className="flex items-center gap-2">
          <CheckCircle2 className="w-4 h-4 text-emerald-500" />
          <span className="text-[11px] font-bold uppercase tracking-wider text-gray-600">
            Completed
          </span>
          <span className="text-[10px] font-bold px-1.5 py-0.5 rounded-full bg-emerald-100 text-emerald-700 min-w-[20px] text-center">
            {items.length}
          </span>
        </div>
        ...
        <button
          ...
          className="flex items-center gap-1 text-xs text-gray-400 hover:text-gray-700 transition-colors select-none"
        >
          {collapsed ? (
            <>
              <span>Show</span>
                ...
                {items.map((item, idx) => {
                  ...
                  return (
                    <tr
                      key={item.id}
                      className={[
                        'cursor-pointer hover:bg-blue-50/30 border-b border-gray-50 transition-colors',
