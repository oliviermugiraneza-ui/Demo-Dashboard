...
export function useConvertBacklog() {
  ...
  const trigger = async (id: number): Promise<{ demoId: number; demo_ref: string | null; item: BacklogItem }> => {
    setLoading(true); setError(null)
    try {
      const res  = await fetch(`/api/backlog/${id}/convert`, { method: 'POST' })
      const json = await res.json() as { ok: boolean; demoId: number; demo_ref?: string | null; data: BacklogItem; error?: string }
      if (!json.ok) throw new Error(json.error ?? 'Convert failed')
      return { demoId: json.demoId, demo_ref: json.demo_ref ?? null, item: json.data }
    } catch (e) {
